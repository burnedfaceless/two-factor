...
class Test_ClassTwoFactorCore extends WP_UnitTestCase {
	...
	/**
	 * Bumping the counter should increase it
	 *
	 * @covers Two_Factor_Core::bump_failed_attempts()
	 */
	public function test_bump_failed_attempts() {
		$user = self::factory()->user->create_and_get();

		$this->assertSame( 0, (int) get_transient( Two_Factor_Core::FAILED_ATTEMPTS_TRANSIENT . $user->ID ) );
		Two_Factor_Core::bump_failed_attempts( $user->ID );
		$this->assertSame( 1, (int) get_transient( Two_Factor_Core::FAILED_ATTEMPTS_TRANSIENT . $user->ID ) );
		Two_Factor_Core::bump_failed_attempts( $user->ID );
		$this->assertSame( 2, (int) get_transient( Two_Factor_Core::FAILED_ATTEMPTS_TRANSIENT . $user->ID ) );
	}
